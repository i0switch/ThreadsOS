...
export class ProfileContextServiceImpl implements ProfileContextService {
  getProfileContext(): ProfileContext | null {
    ...
    return {
      ...
      subNiches: profile.subNiches
        ? profile.subNiches.split(",").map((s) => s.trim())
        : [],
      tone: profile.tone,
      forbiddenTopics: profile.forbiddenTopics
        ? profile.forbiddenTopics.split(",").map((s) => s.trim())
        : [],
  ...
  formatForPrompt(): string {
    ...
    const subNichesStr =
      profile.subNiches.length > 0 ? ` (${profile.subNiches.join(", ")})` : "";
    const toneStr = profile.tone ? `。トーン: ${profile.tone}` : "";
    const forbiddenStr =
      profile.forbiddenTopics.length > 0
        ? `。禁止トピック: ${profile.forbiddenTopics.join(", ")}`
        : "";
    const goalStr = profile.monetizationGoal
      ? `。目標: ${profile.monetizationGoal}`
      : "";
